...
async function cargarEventos() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/eventos`, { headers });
...
function abrirModal(evento) {
  ...
  const btnAplicar = document.getElementById('btnAplicarEventoPage');
  if (btnAplicar) {
    const fechaEvento = new Date(evento.year, evento.Mes - 1, evento.Dia);
    fechaEvento.setHours(0, 0, 0, 0);
    const hoy = new Date();
    hoy.setHours(0, 0, 0, 0);

    if (typeof isAuthenticated === 'function' && isAuthenticated() && typeof isEstudiante === 'function' && isEstudiante() && fechaEvento >= hoy) {
      btnAplicar.style.display = 'inline-flex';
      btnAplicar.onclick = function() { aplicarAEvento(evento, btnAplicar); };
    } else {
...
async function aplicarAEvento(evento, btnElement) {
  const token = getAuthToken();
  if (!token) {
    alert('Debes iniciar sesión para aplicar a un evento.');
    return;
  }
  ...
  try {
    const response = await fetch(`${API_BASE}/api/eventos/${evento.Id_Eventos}/aplicar`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      btnElement.textContent = 'Inscrito!';
      ...
      btnElement.textContent = 'Aplicar a este evento';
      alert(data.error || 'Error al aplicar al evento.');
    }
  } catch (error) {
    btnElement.disabled = false;
    btnElement.textContent = 'Aplicar a este evento';
    console.error('Error al aplicar a evento:', error);
    alert('Error de conexión al aplicar al evento.');
  }
}
